void drawList(){
  ...
  for(int i = 0; i<height/25;i++){
   line(0, 25*i, 200, 25*i); 
  }
}
void displayInList(String name, String category){
  text(name, 10, workingAttractions*12.5);
  text(category, 110, workingAttractions*12.5);
}
...
void loadCity(){
  ...
  for(int i = 0; i<cityLines.length;i++){
    ...
    city c = new city(cityName, position);
    c.checkPicked();
  }
}

int workingAttractions = 0;
...
void loadAttractions() {
  ...
  for (int i = 0; i < lines.length; i++) {
    ...
    PVector position = new PVector(lat, lon);
    attractions a = new attractions(name, rating, position, category);
    a.checkInRange();
    if (a.inRange == true){
      workingAttractions += 1;
      displayInList(a.name, a.category);
    }
  }
}
